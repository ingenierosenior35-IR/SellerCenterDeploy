...
import { useUpdateSubAccount } from './use-edit-subaccount';
...
describe('useUpdateSubAccount', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('should be idle before mutate is called', () => {
    const { result } = renderHook(() => useUpdateSubAccount(), {
  ...
  it('should invalidate getSubSellerList queries on success', async () => {
    ...
    const { result } = renderHook(() => useUpdateSubAccount(), { wrapper });
